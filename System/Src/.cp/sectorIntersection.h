#ifndef SECTOR_INTERSECTION_H
#define SECTOR_INTERSECTION_H
...
SortedDynamicArray<FireArea::Ray> *rqwqw = new SortedDynamicArray<FireArea::Ray>;
Iterator<FireArea::Points> point = (*rqwqw)[0].pointOfIntersection.begin();

		for (i = 0; i <= n - 1; i++){
			for (
					Iterator<FireArea::Points> point = rays[i].pointOfIntersection.begin();
					point != rays[i].pointOfIntersection.end();
					++point){
				if (!(*point).PositivePoint){
					int locCount = 0;
					for (auto locPoint = rays[i].pointOfIntersection.begin(); locPoint != rays[i].pointOfIntersection.end(); ++locPoint){// �������� ��������
