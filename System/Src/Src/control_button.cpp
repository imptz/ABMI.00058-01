...
Button::Button(unsigned int _positionX, unsigned int _positionY, unsigned int _width, unsigned int _height, const char* _name, Window::BORDER_STYLE _borderStyle, MessageReceiver* _messageReceiver)
	:	Control(_positionX, _positionY, _width, _height, _messageReceiver), ITimer(TIMER_PERIOD), state(BUTTON_STATE_UP), downCounter(0), borderStyle(_borderStyle), hold(false)
{
	...
	setName(const_cast<char*>(_name), false);
