...
ConfigData::ConfigData(unsigned int _BUFFER_SIZE)
	:	ITimer(TIMER_PERIOD), cData(nullptr), cDataNew(nullptr), dataSave(false), phase(CONFIG_DATA_PHASE_READ_1), BUFFER_SIZE(_BUFFER_SIZE),
	 	buffer(new unsigned char[BUFFER_SIZE]), taskId(0)
{
	pTimer->start();
}
...
print("CONFIG_DATA_PHASE_READ_1_1", 40, getNextPos());
Cpu::cli();
for(;;);
			taskId = HddManager::getSingleton().read(buffer, START_SECTOR, BUFFER_SIZE / 512);
			if (taskId != HddManager::UNDEFINED_ID){
print("CONFIG_DATA_PHASE_READ_1_2", 40, getNextPos());
Cpu::cli();
for(;;);
...
int ConfigData::getConfigDataStructInitSignalsIndexByNumber(unsigned int number)
{
...
{
//SERIAL_DEBUG_ADD_DEBUG_MESSAGE_STRING1("getConfigDataStructProgramInitSignal___initSignalNumber:  ", initSignalNumber);

	unsigned int count = 0;
	for (unsigned int i = 0; i < cData->number_Programs; i++)
	{
//SERIAL_DEBUG_ADD_DEBUG_MESSAGE_STRING1("cData->_Programs[i]->initSignalNumber:  ", cData->_Programs[i]->initSignalNumber);
//SERIAL_DEBUG_ADD_DEBUG_MESSAGE_STRING1("cData->_Programs[i]->function:  ", cData->_Programs[i]->function);
...
//SERIAL_DEBUG_ADD_DEBUG_MESSAGE_STRING1("resCount_____________:  ", count);
...
//		if ((cData->_Programs[i]->initSignalNumber == initSignalNumber) && (cData->_Programs[i]->function == _function))
		...
		{
			(*pList)[resultIndex++] = i;
//SERIAL_DEBUG_ADD_DEBUG_MESSAGE_STRING2("getConfigDataStructProgramInitSignal:  ", cData->_Programs[i]->prNumber, initSignalNumber);
		}
