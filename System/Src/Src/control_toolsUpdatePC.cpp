...
char* ToolsUpdatePC::TOOLS_UPDATE_PC_MESSAGE_FINISH = const_cast<char*>(LOCAL_TOOLS_UPDATE_PC_MESSAGE_FINISH_TEXT);

ToolsUpdatePC::ToolsUpdatePC(unsigned int _positionX, unsigned int _positionY, MessageReceiver* _messageReceiver)
	:	Control(_positionX, _positionY, WIDTH, HEIGHT, _messageReceiver), ITimer(TIMER_PERIOD), updatePhase(-1){
